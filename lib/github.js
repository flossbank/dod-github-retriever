          ...
          async (response, retry) => {
            if (response.statusCode === 403) {
              if (!response.headers['retry-after']) {
                this.log.warn('Received 403 status code without retry-after header')
                this.log.warn(response.headers)
                this.log.warn(response.body)
                return response
              }

              // get the Retry-After header and sleep
              const retryAfterSeconds = parseInt(response.headers['retry-after'], 10)
              const retryAfterDate = new Date()
              retryAfterDate.setTime(Date.now() + (retryAfterSeconds * 1000))

              this.log.warn('Secondary rate limits reached; sleeping for %d seconds (%s)',
                retryAfterSeconds,
                retryAfterDate.toString()
              )
              await sleepUntil(retryAfterDate)
            ...
            this.log.warn('Current rate limit status (%s): [remaining: %d]; [reset: %s]',
              response.requestUrl,
