          ...
          async (response, retry) => {
            if (response.statusCode === 403) {
              if (!response.headers['x-ratelimit-reset']) {
                this.log.warn('Received 403 status code without x-ratelimit-reset header, aborting')
                return response
              }

              // get the reset header and sleep
              const resetTimestamp = new Date(parseInt(response.headers['x-ratelimit-reset'], 10) * 1000)
              this.log.warn('Secondary rate limits reached; sleeping until %s', resetTimestamp.toString())
              await sleepUntil(resetTimestamp)
