...
exports.process = async ({ log, record, db, retriever, resolver, dynamo }) => {
  ...
  // now that we have top level packages for each supported registry/language, we create a file in s3
  // with the correlation ID and send off an event to dod-registry-resolver

  // TODO write to s3

  // TODO send off event to RR

  // all done! unlock on correlation id
